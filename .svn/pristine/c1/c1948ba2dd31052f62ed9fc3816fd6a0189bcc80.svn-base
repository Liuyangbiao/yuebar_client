...
public class AsyncHttpClient {
	...
	public void postJsonParams(String url, RequestParams params,
			AsyncHttpResponseHandler responseHandler) {
		RequestParams newParams = new RequestParams();
		JSONObject jsonObject = new JSONObject();
		JSONObject jsonPage = new JSONObject();
		for (BasicNameValuePair param : params.getParamsList()) {
			if (!"action".equals(param.getName())) {
				jsonObject.put(param.getName(), param.getValue());
			} else {
				newParams.put("action", param.getValue());

			}
			if ("pageindex".equals(param.getName())) {
				jsonPage.put(param.getName(), param.getValue());
			}
			if ("pagecount".equals(param.getName())) {
				jsonPage.put(param.getName(), param.getValue());
			}
		}
		Log.d("AsyncHttpClient", jsonObject.toString());
		newParams.fileParams = params.fileParams;
		newParams.put("param", jsonObject.toString());
		if (jsonPage.getString("pageindex") != null
				&& jsonPage.getString("pagecount") != null) {
			newParams.put("page", jsonPage.toString());
		}

		Log.d(ttag, jsonObject.toString());
		Log.d(ttag, jsonPage.toString());
		Log.d(ttag, newParams.toString());

		post(null, url, newParams, responseHandler);
	}
