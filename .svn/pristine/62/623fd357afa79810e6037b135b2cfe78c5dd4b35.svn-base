...
public class VoiceCallActivity extends BaseActivity implements OnClickListener {
	...
	public VoiceCallActivity() {
		super(R.layout.activity_voice_call);
	}
		...
		if (!isInComingCall) {// 拨打电话
			...
			try {
				...
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						Toast.makeText(VoiceCallActivity.this, "尚未连接至服务器", Toast.LENGTH_SHORT).show();
					}
				});
			}
		} else { // 有电话进来
	...
	private void saveCallRecord() {
		...
		// 设置扩展属性
		message.setAttribute(GlobleConstant.MESSAGE_ATTR_IS_VOICE_CALL, true);
